...
cat > /etc/duckdns-credentials.ini <<EOF
dns_duckdns_token = ${DUCKDNS_TOKEN}
EOF
chmod 600 /etc/duckdns-credentials.ini
...
if [ -f "/etc/letsencrypt/live/${DUCKDNS_DOMAIN}/fullchain.pem" ]; then
    ...
    certbot certonly \
        ...
        --dns-duckdns-credentials /etc/duckdns-credentials.ini \
...
echo "0 2 * * * certbot renew --quiet --deploy-hook 'nginx -s reload' 2>&1 | logger -t certbot" | crontab -
