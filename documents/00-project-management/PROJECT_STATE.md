...
## REST Endpoints
There are exactly 43 registered HTTP REST endpoints plus 2 WebSocket endpoints (`/ws` for the global queue, `/ws/rooms/{slug}` for per-room queue deltas introduced in R07b, playback deltas added in R09a, and room vote deltas added in R09b):
...
43. `POST /api/rooms/{slug}/vote/skip` — any active room member, in-memory strict-majority vote session (30s expiry, max(2, n/2) threshold) for the current song. On passed vote the queue advances to the next song and broadcasts room_playback_song_advanced reason=skip. Bypasses player-lease holder rule.
...
- **Room-scoped vote sessions (R09b, additive):** Vote sessions are stored in-memory in `internal/usecase/roomvote.Interactor`. Sessions are keyed `skip:{roomSlug}:{songID}`, scoped to the current song, expire after 30 seconds, are single-instance only, and never persisted. Threshold = `max(2, uniqueConnectedUserIDs(slug) / 2)` captured at session creation via `(*RoomWSHub).UniqueConnectedUserIDs(slug)`, which de-duplicates by `userID` so a user with multiple tabs counts once. The global session map, global vote thresholds, priority balances, and auto-queue are all unchanged.
