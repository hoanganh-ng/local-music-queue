...
## REST Endpoints
There are exactly 47 registered HTTP REST endpoints plus 2 WebSocket endpoints (`/ws` for the global queue, `/ws/rooms/{slug}` for per-room queue deltas introduced in R07b, playback deltas added in R09a, room vote deltas added in R09b, room playback volume delta added in R09c, room playback previous delta added in R09d, and room auto-queue events added in R09f):
