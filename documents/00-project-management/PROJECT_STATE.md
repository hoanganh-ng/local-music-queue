...
## REST Endpoints
There are exactly 44 registered HTTP REST endpoints plus 2 WebSocket endpoints (`/ws` for the global queue, `/ws/rooms/{slug}` for per-room queue deltas introduced in R07b, playback deltas added in R09a, and room vote deltas added in R09b):
