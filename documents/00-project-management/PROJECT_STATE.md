...
**Branch:** `dev`\
**Current active sprint:** **Sprint 036 — R14 Discard-and-Retire Implementation-Correction Shaping** ([`SPRINTS/036-r14-discard-retire-implementation-correction-shaping.md`](./SPRINTS/036-r14-discard-retire-implementation-correction-shaping.md)) — the sole active, documentation-only shaping sprint (Issue #17 comments #5140909519, #5140909749, #5140910084, #5140914484). It shapes, scopes, and defines acceptance criteria for the implementation-correction sprint required by ADR 004 Section 6; Product Owner acceptance of its PR authorizes (does not activate) that sprint, which remains not active and not started until separately activated with its own record and Builder authorization. Sprint 035 — R14 Discard-and-Retire Lifecycle Transition ([`SPRINTS/035-r14-discard-retire-lifecycle-transition.md`](./SPRINTS/035-r14-discard-retire-lifecycle-transition.md)) — the completed documentation-only lifecycle bridge (Issue #17 comments #5138919618, #5138932156, #5138980052, #5138989366) — records that Sprint 034 — R14 Discard-and-Retire Contract Amendment ([`SPRINTS/034-r14-discard-and-retire-contract-amendment.md`](./SPRINTS/034-r14-discard-and-retire-contract-amendment.md)) is **closed and accepted** (PR #32, accepted head `6686852445d3cf46bf6c46f3b76d94ba9b932098`, squash-merged into `dev` as `e65eef9954b2f4ba24f4866b2b084910e76e090c` on 2026-07-31) and that ADR 004 — Discard legacy global state at room cutover ([`ADRS/004-discard-legacy-global-state-at-room-cutover.md`](./ADRS/004-discard-legacy-global-state-at-room-cutover.md)) is **accepted architecture authority on `dev`**, superseding the migrate-and-retire production assumptions of ADR 003, R14b, and R14c. Sprint 036 authorizes documentation work only — no implementation, preflight, or production work. Sprint 031 — R14c Gate 2 Preflight and Go/No-Go Preparation ([`SPRINTS/031-r14c-gate2-preflight.md`](./SPRINTS/031-r14c-gate2-preflight.md)) remains **paused** (not closed, not superseded, not resumed). Nothing is resolved: all readiness rows remain `Not started`, the unsigned recommendation remains `DEFER / NOT READY`, and Gate 2 remains pending and unauthorized.
...
Consequences: the accepted `cmd/room-cutover` copy mechanism (R14b) is declared unsuitable for the revised production cutover (it remains an accepted historical artifact); the current activation proof is explicitly unresolved (the startup guard depends on `room_cutover_marker`, which proves a migrated-room copy); the runbook and all Gate 2 documents are marked **non-executable** until a later implementation-correction sprint (ADR 004 Section 6 — being shaped by Sprint 036, pending acceptance; NOT active; activation requires its own Product Owner approval) is accepted and integrated and the revised Gate 2 preflight is separately resumed.
...
Sprint 035 — **R14 Discard-and-Retire Lifecycle Transition** ([`SPRINTS/035-r14-discard-retire-lifecycle-transition.md`](./SPRINTS/035-r14-discard-retire-lifecycle-transition.md)) — is the **completed documentation-only lifecycle bridge** (Issue #17 comments #5138919618, #5138932156, #5138980052, #5138989366) that reconciled the trackers with `dev` after PR #32's acceptance: Sprint 034 is closed and accepted, ADR 004 is accepted architecture authority, and **no sprint is active** after the transition (a state since superseded by Sprint 036's activation). It changed no governance rule, altered no product direction, touched no deployment document, and resolved nothing: Sprint 031 remains paused, the implementation-correction sprint left by the transition as required but unshaped, inactive, and unauthorized (since being shaped by Sprint 036, pending acceptance), all readiness rows remain `Not started`, the evidence and discrepancy inventories remain empty, all decisions remain unsigned, the unsigned recommendation remains `DEFER / NOT READY`, Gate 2 remains pending and unauthorized, production remains untouched, and R14e remains inactive.
...
R14c Gate 1 implementation and rehearsal are integrated and closed on `dev`. Per ADR 004 (Sprint 034, 2026-07-30), the production direction is now **discard-and-retire**: the migrate-and-retire production execution that R14c Gate 2 was preparing **will not run**. Sprint 031's Gate 2 preflight is paused (not closed, not superseded) and its documents are non-executable. A later implementation-correction sprint (ADR 004 Section 6 — being shaped by Sprint 036, pending acceptance; NOT active; activation requires its own Product Owner approval) must land the revised activation proof, cutover procedure, and operational surfaces before any revised Gate 2 preflight may resume. R14e remains inactive.
