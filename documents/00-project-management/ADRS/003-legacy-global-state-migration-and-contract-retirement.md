...
- Date: 2026-07-15 (revised three times after Architect reviews; third-pass corrections applied 2026-07-15)
...
> **Fourth-pass review-revision summary (2026-07-15).** After the fourth Architect review (corrective Builder prompt on Issue #20), this ADR applies 6 corrective changes from the third-pass draft, plus the cumulative 14 second-pass corrections, the cumulative 14 first-pass corrections, and the 5 third-pass corrections: (1) **sprint execution order is `R05b → R09h → R14b → R09i → R14d → R14c → R14e`** — R09i follows R14b because R14b's `0009_room_cutover_support.up.sql` migration creates `room_activities` that R09i wires; (2) the **new migration `0009_room_cutover_support.{up,down}.sql` creates BOTH `room_activities` AND `room_cutover_marker`** (and their indexes / constraints / `room_cutover_marker` PK only) — the CLI does NOT create schema ad hoc; the `.down.sql` drops both tables cleanly; (3) **the marker table stores `target_room_id` and `host_user_id` as immutable integer audit snapshots WITHOUT foreign keys** — explicitly NOT `REFERENCES rooms(id)` / `REFERENCES users(id)` — because a FK would permanently own the lifecycle of the room or the user and prevent future hard-delete / retention work that R10f+ defers; migration evidence must NOT own room or user lifecycle; (4) **the marker timestamp field `cutover_pre_commit_at` is supplied explicitly by the CLI** as a parameter (CLI process clock at marker insertion) rather than relying on `DEFAULT NOW()`; the CLI's clock is the authoritative recorder, and `DEFAULT NOW()` would silently use the DB clock at insert time; (5) **R09i write behavior must remain DISABLED until R14c has copied legacy activities and resynchronized `room_activities_id_seq`** — this is the activity-ID collision guard (symmetric with the `legacy_id_offset` mechanism for `room_play_history`); without it, both the legacy `activities` table (still writable by the global runtime during Phase A/B) and the new `room_activities` table (BIGSERIAL) could allocate overlapping IDs concurrently; R09i's gating uses the same single cutover-state mechanism as R14d's behavioral activation; (6) **the frontend cutover-state gate is a single named mechanism: `VITE_ROOM_CUTOVER_AUTHORITATIVE`** (Vite build-time `import.meta.env` baked at SPA build); two bundle variants — `false` (pre-R14c / paired with server `--room-cutover-authoritative=false`) and `true` (post-R14c / paired with server `--room-cutover-authoritative=true`); there is **NO runtime feature flag** — the SPA is built twice and the right bundle is deployed for the right server state; no alternate mechanism (no separate runtime env var, no separate feature-flag service) is permitted within R14a's contract; (7) also from the third-pass: `cmd/room-cutover` ships NO `abort` subcommand (PostgreSQL session locks belong to the connection that acquired them; lock release via deferred `pg_advisory_unlock` on the same pinned connection with connection close as fallback); the marker is inserted in the same transaction as the room + host + copy; (8) also from the third-pass: every new migration ships a matching `.down.sql`; (9) from the fourth-pass: **the `0010_drop_legacy_global_tables.down.sql` MUST restore the exact legacy schema structure** — columns, indexes, sequences, constraints, defaults, and the required `auto_queue_config` singleton seed row (id=1) — so rolling forward/backward on the legacy tables is a no-op on structure; the .down.sql MAY also restore the singleton `queue_state` row as an empty shell; the documented default is that **dropped data cannot be restored** (the destructive cleanup is one-way); this is documented in the `.down.sql` file header.
...
- `0009_room_cutover_support.up.sql` and `0009_room_cutover_support.down.sql` — create / drop both `room_activities` AND `room_cutover_marker` (with indexes, constraints, PK, FKs).
...
- R14e lands `0010_drop_legacy_global_tables.up.sql` — drops the legacy global tables (NOT `room_play_history`, NOT `room_activities`). Schema becomes **v10**.
- **`0010_drop_legacy_global_tables.down.sql` MUST restore the exact legacy schema structure** so that rolling forward/backward on the legacy tables is a no-op on structure. Specifically, the `.down.sql` MUST recreate:
  - All four legacy tables with their exact column structure (column names, types, nullability, defaults) as declared in `0001_initial.up.sql`.
  - All legacy **indexes** declared in `0001_initial.up.sql` (the index on `play_history.played_at`, the unique constraints on `users.email`, `user_sessions(user_id, session_date)`, etc.).
  - All legacy **sequences** (`users_id_seq`, `user_sessions_id_seq`, `priority_transactions_id_seq`, `activities_id_seq`, `play_history_id_seq`); the recreated tables are wired to those sequences via `BIGSERIAL` / `DEFAULT nextval(...)` defaults so legacy writes resume from the same id space.
  - All legacy **constraints** (PK / UNIQUE / NOT NULL / CHECK) as declared in `0001_initial.up.sql` (e.g., `queue_state.id = 1` CHECK).
  - All legacy **column defaults** (e.g., `users.created_at`, `queue_state.id = 1`).
  - The required **`auto_queue_config` singleton seed row** (`id = 1` with the `enabled`, `strategy` columns at their initial values), so the global autoplay toggle works against the recreated table — `INSERT INTO auto_queue_config (id, enabled, strategy) VALUES (1, ...) ON CONFLICT DO NOTHING`.
- The `.down.sql` MAY also restore the singleton **queue_state row** (`id = 1` with `data = NULL` or the documented default empty JSON), but the documented default is that **`0010.down.sql` cannot restore dropped data** — any non-null `data` payload that existed before `0010.up.sql` ran is lost. The recreated `queue_state` row is an empty shell; the payload is recoverable only by re-running `0010.up.sql` (the destructive cleanup is one-way). This is documented in the `.down.sql` file header.
...
- `cmd/room-cutover` ships subcommands `plan`, `up`, `verify` — **no `abort` subcommand** (PostgreSQL session locks belong to the connection that acquired them; release is via `pg_advisory_unlock` on the same pinned connection, with connection close as fallback).
- `room_cutover_marker` table created by `0009_room_cutover_support.up.sql`; PK = 1; FKs to `rooms(id)` and `users(id)` both `ON DELETE RESTRICT`.
- Binary build flag `--room-cutover-authoritative` (default `false`).
...
The cutover involves a **single named gate mechanism on each side**:
- **Server side**: `--room-cutover-authoritative=true|false` (build-time Go flag baked at compile; default `false`).
- **Client side**: `VITE_ROOM_CUTOVER_AUTHORITATIVE=true|false` (Vite build-time `import.meta.env` value baked at SPA build; two bundle variants — `false` bundle shipped pre-R14c, `true` bundle shipped as part of the R14c rollout).
- There is **no runtime feature flag**: the server's `--room-cutover-authoritative` value and the SPA's `VITE_ROOM_CUTOVER_AUTHORITATIVE` value are baked at build and **must match** in any deployed combination. The matrix below names this single mechanism explicitly.
...
- ADR 003 status remains `Proposed` (NOT `Accepted`).
