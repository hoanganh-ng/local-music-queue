...
- Status: Accepted — R14a contract; supersedes parts of ADR 001 and ADR 002 §11/§13; Product Owner acceptance 2026-07-16. **Partially superseded by ADR 004 (2026-07-30):** the migrate-and-retire production portions of this contract — the migrated Product-Owner-named room, the `cmd/room-cutover` production copy of the legacy global `queue_state`, `activities`, `auto_queue_config`, and `play_history`, the `legacy_id_offset` / `room_activities_id_seq` resequencing semantics, and the migrated-room production-window execution assumptions — are superseded by ADR 004 ([`004-discard-legacy-global-state-at-room-cutover.md`](./004-discard-legacy-global-state-at-room-cutover.md)): at the revised cutover the legacy global state is **discarded, not migrated**, and no migrated room is created. The retirement intent (tombstoned legacy contracts, room-authoritative runtime, fail-closed activation, mandatory backups, false/false rollback pair, eventual R14e cleanup) remains in force. This ADR's body is preserved unchanged below as the historical record of the accepted R14a contract and of the R14b/R14c Gate 1 implementations built against it.
