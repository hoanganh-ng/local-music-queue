...
> **Third-pass review-revision summary (2026-07-15).** After the third Architect review (corrective Builder prompt on Issue #20), this ADR applies 5 corrective changes from the second-pass draft, plus the cumulative 14 second-pass corrections and the cumulative 14 first-pass corrections: (1) **sprint execution order is `R05b → R09h → R14b → R09i → R14d → R14c → R14e`** — R09i follows R14b because R14b's `0009_room_cutover_support.up.sql` migration creates `room_activities` that R09i wires; (2) the **new migration `0009_room_cutover_support.{up,down}.sql` creates BOTH `room_activities` AND `room_cutover_marker`** (and their indexes / constraints / `room_cutover_marker` PK + FK to `rooms` + `users`) — the CLI does NOT create schema ad hoc; the `.down.sql` drops both tables cleanly; (3) **R14d's behavioral activation is deferred to the R14c maintenance window** — R14d may be implemented and accepted before R14c, but the SPA's behavior change (removing `DashboardView`, clearing global slices, closing global WS) activates only during the R14c cutover or behind an explicit cutover-state gate; pre-R14c the SPA continues exposing the legacy dashboard surface so the still-authoritative global queue remains reachable; (4) the **lock contract uses deferred `pg_advisory_unlock(987654321)` on the same pinned connection** (mirroring R03's idiom) — `cmd/room-cutover` ships NO `abort` subcommand; the marker timestamp field is **`cutover_pre_commit_at`** (NOT `cutover_committed_at`) because the row is written before `COMMIT` and cannot truthfully reflect the DB commit time; (5) **every new migration ships a matching `.down.sql`** following the R00–R11a convention; the **`0010_drop_legacy_global_tables.down.sql` cannot restore dropped data** (it only recreates empty legacy table shells) and the destructive-cleanup one-way behaviour is documented in the `.down.sql` header.
...
The field is named `cutover_pre_commit_at` (NOT `cutover_committed_at`) because the row is written before `COMMIT` and cannot truthfully reflect the DB commit time. The field records the pre-commit completion timestamp.
...
The cutover involves two build flags (`--room-cutover-authoritative` per-server, R14d's cutover-state gate per-client).

| Server binary | SPA cutover-state gate | Legacy global routes | Per-room routes | Result |
| --- | --- | --- | --- | --- |
| Pre-R14b binary | OFF | Registered; serve legacy repos | Registered | Pre-cutover baseline. |
...
| Post-R14c binary (`--room-cutover-authoritative=true`) | ON (gate flipped during maintenance) | **Tombstone handlers; `410 Gone` envelope + `Link`** | Registered; sole authoritative path | Production cutover state. ONLY valid post-R14c combination. |
| Mid-window rollback: immediately pre-cutover production release with `--room-cutover-authoritative=false` | OFF | Registered; serves legacy repos | Registered | Mid-window rollback state. The `pg_dump` from the start of the window is the source of truth. Forward recovery for any data that mutated after the cutover. **NOT an "R03 binary"** — R03 is the unrelated SQLite-to-PostgreSQL migration release. |

**Client-continuity claim:** the only combinations that guarantee client continuity are the documented ones. R14d's behavioral activation is gated. Pre-R14c the SPA exposes the legacy dashboard. The compatibility matrix forbids deploying the post-R14c server with the pre-R14d SPA.
