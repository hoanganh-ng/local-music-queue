...
**Supersession note (2026-07-15, R14a):** Parts of this ADR are now historically inaccurate and have been superseded by [`003-legacy-global-state-migration-and-contract-retirement.md`](./003-legacy-global-state-migration-and-contract-retirement.md) (ADR 003, Proposed — pending Architect review and Product Owner acceptance). ADR 003 supersedes ONLY the following paragraphs of ADR 001, and nothing else: §3 Decision 6 (compatibility shim never built); §9 transition-strategy table (R07 row never built) + final route shape list (`{roomId}` vs `{slug}` + routes never built); §10 (`/ws` synthesized default-room paragraph never built); §11 (R06 fold attribution — R06 became Player Lease) + the "Room-scoped activities" row (no `room_activities` table exists); §15 compatibility table rows referencing the shim and default `/ws`; §18 sprint → scope map (out of date). ADR 001 body of §3–§18 is unchanged byte-for-byte; this note is the only ADR 001 edit. ADR 001 remains authoritative for all decisions not listed here, including explicit rooms / no permanent default room (§3 Decision 2), room lifecycle (§4), membership roles (§3 Decision 4), one-host-per-active-room (§3 Decision 4), player-lease separation (§3 Decision 5), invite-token principles (§7), per-room REST/WebSocket direction (§3 Decision 6 + §10), single-instance limitations (§16), and account-scoped priority (§3 Decision 9). The implementation-ready migration + retirement contract lives in [`../SPRINTS/022-legacy-global-state-migration-and-contract-retirement-plan.md`](../SPRINTS/022-legacy-global-state-migration-and-contract-retirement-plan.md).
