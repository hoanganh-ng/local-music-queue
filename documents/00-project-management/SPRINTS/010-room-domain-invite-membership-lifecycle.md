...
- **Archived-room mutation rules:** no new invite creation, no invite redemption (via active-room gate in the interactor), no member promotion, and no member demotion on archived rooms; only GET and DELETE (archive) allowed. The same `requireActiveRoom` gate in the interactor also scopes the host-only listing path: `interactor.ListInvites` and `interactor.ListMembers` are rejected with 409 Conflict when the room is archived, even for a valid host. As a side effect of the gate ordering (host check before active-room check in the interactor), a non-host caller hitting `ListInvites` on an archived room receives 403 Forbidden from `requireHost` rather than 409 from `requireActiveRoom` — host check fires first because it has no prerequisite on room state. For audit, the underlying `rooms`, `room_members`, and `room_invites` rows persist in PostgreSQL after archive; only the interactor's `requireActiveRoom` gate prevents reads via the public API, so archived-room audit data is recoverable by a future admin read path that bypasses the gate.
