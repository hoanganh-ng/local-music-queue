...
**Status:** Active — implementation and isolated-rehearsal gate only (Product Owner approved 2026-07-28)  
...
- **Review-head sequence:** `1502c3a243b3350b6a18ff2454118981f4032357` (initial Gate 1) → `0daa8fbe22b896c6def5f2e75af129792ee41229` (first corrective pass, findings F1–F6) → the new corrective head from this pass (exact SHA posted to PR #26 at push; a commit cannot embed its own hash). The actual changed-file count is the GitHub PR #26 file count for the current head.
- **Status:** Gate 1 implementation **pending re-review**. The first corrective pass (`0daa8fb…`) addressed findings F1–F6. This pass addresses the follow-up items: pre-window snapshot `plan` and isolated `up --dry-run` executed through the packaged backend image with `<EVIDENCE_DIR>` bind-mounted to separate durable `0600` reports; parameterized (`psql -v :'…'`) identity SQL replacing direct interpolation, with login eligibility, account-level `users.role`, and cutover-created room-host membership documented as separate concerns; rollback image IDs/digests captured from the running containers before any rebuild or prune; and the Compose pairing test skipping only when Docker/Compose is unavailable while failing on config-render errors. None alter the accepted runtime flag, startup guard, activity-writer selection, or tombstone handler.
- **Corrective head:** the Builder submits an unmerged corrective commit on the existing branch and posts its exact SHA to PR #26. The Builder does not push, merge, or advance the sprint; repository history and sprint advancement remain with the Product Owner.
