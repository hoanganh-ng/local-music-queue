...
**Closure:** R07d closed 2026-06-30 and accepted by the Product Owner. The closure pass tightens three narrow invariants introduced by the original R07d landing, all on `dev`, no out-of-scope work. **Blocking fix (handler request shape).** `roomQueuePrioritizeReq.SongIndex` is now `*int` so a missing field is detectable. `HandlePrioritizeRoomSong` rejects `nil` and negative values with `400 Bad Request` and forwards `*req.SongIndex` to the interactor. The pre-fix `int` form silently defaulted missing fields to 0, which used to mutate the song at index 0; the closure test `TestRoomQueue_PrioritizeSong_MissingBodyReturns400` seeds a queue with `current_index=1` (not 0) and asserts `400` plus a byte-for-byte unchanged persisted queue and zero broadcasts. **Important fix (broadcast payload).** `Interactor.PrioritizeSong` now returns `queue.Songs[toIndex]` (the post-mutation entity with `IsPrioritized=true`) instead of the pre-mutation snapshot. `TestRoomQueue_PrioritizeSong_ReturnedSongIsPrioritizedTrue` pins the interactor-level invariant; `TestRoomQueue_PrioritizeSong_BroadcastSongIsPrioritizedTrue` pins the handler/broadcast seam. **Docs and frontend cleanup.** `documents/00-project-management/PROJECT_STATE.md` REST count moves 37 → 38 and adds `POST /api/rooms/{slug}/queue/prioritize`; the per-room WebSocket inventory now lists `room_queue_song_prioritized` and no longer claims "exactly four" event types. `frontend/src/store/index.js` `applyRoomSongPrioritized` fallback stamps `is_prioritized: true` (snake-case, matching the entity JSON tag) instead of `IsPrioritized: true`, and the corresponding tests assert `is_prioritized`. **No preserved invariant was relaxed.** Global `/api/queue/prioritize` is untouched, no priority balance spending, no relational queue rows, no mutation of the global `queueState` / `currentUser` / `voteSessions` / `autoQueueConfig` from room events, and per-room seq allocation remains hub-loop owned (`dispatch()` does not allocate seq).
