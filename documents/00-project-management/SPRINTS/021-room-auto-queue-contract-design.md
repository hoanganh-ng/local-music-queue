...
R09f is the backend-only runtime implementation slice that conforms to the R09e contract. Implemented on `dev` on 2026-07-01 (immediately after R09e) and **accepted by the Product Owner on 2026-07-01** (the closure pass landed the same day). R09f lands:
...
- **Queue-owned conditional insertion (`roomqueue.Interactor.AddRoomAutoQueueSong`):** runs under the existing roomqueue mutex, revalidates source-song identity + "current is last" + duplicate guard, returns `roomqueue.ErrRoomAutoQueueStale` on any violation with zero side effects. Adapter translates the sentinel to `roomautoqueue.ErrAutoQueueStale` at the wiring boundary. Mirrors the global `queue.AddAutoQueueSong` contract.
- **Async trigger wiring:** the roomqueue interactor fires a non-blocking `CheckAndTrigger(slug)` from `PlaybackEnded`, `SkipPlayback`, `SkipVote`, `RemoveSong`, and `ClearQueue` only when the post-mutation queue is at `current==last`. `PrevPlayback`, `PrioritizeSong`, `AddSong`, `ChangePlaybackVolume`, `SyncPlaybackElapsed`, `SetPlaybackStatus`, and no-mutation error paths do NOT trigger.
- **New use case (`internal/usecase/roomautoqueue/interactor.go`):** holds a coordinator `mu` mutex guarding a per-room `inFlight map[roomID]bool`. The slow `FetchRelated` is held OUTSIDE mu. Per-room in-flight state is independent (a slow fetch for room A does not suppress room B's trigger). `AppendHistory` runs after the mu is released. Disabled-mid-flight + stale variants mirror the global autoqueue Sprint 004 contract.
- **SetEnabled serialization (closure pass):** `SetEnabled`'s `SaveConfig` write now runs under the same coordinator `mu` that `CheckAndTrigger`'s post-fetch recheck + `AddRoomAutoQueueSong` insertion hold. `GetConfig` (and all other slow paths in both methods) remains OUTSIDE the mutex; the critical section is the minimum needed to serialize the disable write with the post-fetch decision window. Prior to the closure pass, `SetEnabled` resolved room + checked role + called `GetConfig`/`SaveConfig` without ever taking the coordinator mutex, which left a window for a `SetEnabled(false)` save to interleave between the final "still enabled?" check and the queue mutation. The package-level `Locking discipline` comment and the `Interactor.mu` field doc reflect the new serialization surface. The disabled-mid-flight test (which had routed through the unconditional `ErrAutoQueueStale` add-fn path) was replaced with two tests that pin the locked-correct outcomes directly — `TestSetEnabled_SerializedWithPostFetchCriticalSection` (asserts `SaveConfig` does not enter while the add fn holds mu, then proceeds after release) and `TestSetEnabled_RunsBeforePostFetchRecheck_DropsCandidate` (asserts the other valid outcome: `SetEnabled(false)` committed before the post-fetch recheck → recheck observes disabled and drops, no add fn call, no queue mutation, no broadcast, no history append).
- **REST endpoints:** `GET /api/rooms/{slug}/autoqueue/status` (any active member; 200 with `{enabled,strategy}`) and `POST /api/rooms/{slug}/autoqueue/toggle` (host/admin only; body `{enabled: *bool}`; missing field returns 400). Status mapping: 200 / 400 / 401 / 403 / 404 / 409.
- **Per-room WebSocket events (additive, `/ws/rooms/{slug}` only):** `room_auto_queue_added` and `room_auto_queue_config_changed`. Constants in `internal/delivery/ws/events.go`; dispatch methods on `*ws.RoomWSHub`; payloads in `RoomAutoQueueAddedData` (room_slug + song + source_song_title + state) and `RoomAutoQueueConfigChangedData` (room_slug + enabled + strategy). The global `/ws` 16-event inventory is unchanged.
- **Frontend (R09f):** intentionally deferred — R09f is backend-only per the sprint directive. The frontend RoomView, globalStore.roomQueues[slug].autoQueueConfig slice, `applyRoomAutoQueueAdded` mutator, `applyRoomAutoQueueConfigChanged` mutator, `room_auto_queue_added`/`room_auto_queue_config_changed` WS listeners, and the `Radio` toggle UI are NOT implemented in R09f.

## Verification (R09f)

- `go test -count=1 ./internal/usecase/roomautoqueue/... ./internal/usecase/roomqueue/... ./internal/delivery/ws/... ./internal/delivery/http/... ./internal/infrastructure/persistence/... ./cmd/server/...` — PASS.
- `go test -race -count=1 ./internal/usecase/roomautoqueue/... ./internal/usecase/roomqueue/... ./internal/delivery/ws/...` — PASS.
