...
Sprint 014 / R09a — Lease-aware room playback controls is the currently active sprint (in progress on `dev` — not yet accepted by the Product Owner). R09a adds four lease-holder-only room playback mutations (`POST /api/rooms/{slug}/playback/{status,sync,skip,ended}`), three additive per-room WebSocket events (`room_playback_status_changed`, `room_playback_elapsed_sync`, `room_playback_song_advanced`), and a minimal RoomView Play/Pause/Skip/Ended control surface. Lease enforcement lives at the use-case layer via a new `room.PlaybackLeaseAuthorizer` seam implemented by `PlayerLeaseInteractor.RequireActiveLeaseHolder`. The global `/api/queue/...` contract, the 16-event global `/ws` inventory, voting, auto-queue, public API/CORS, Docker, and auth remain untouched. See the *Implementation summary (R09a)* section in [014-player-control-semantics.md](./014-player-control-semantics.md).

Sprint 010 / R04 — Room Domain, Invite, Membership, and Lifecycle is the previously authorized sprint (pending Product Owner acceptance of the R04 closure pass). See [010-room-domain-invite-membership-lifecycle.md](./010-room-domain-invite-membership-lifecycle.md) for the sprint record.
