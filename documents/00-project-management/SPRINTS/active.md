...
Sprint 012 / R07a — Room-scoped playback queue (narrow first slice) is closed (2026-06-29, accepted by the Product Owner). See [012-room-scoped-playback-queue.md](./012-room-scoped-playback-queue.md). Full R07 remains split; R07b (per-room WebSocket sync/delta events at `GET /ws/rooms/{slug}`) is implemented on `dev` and pending Architect/Product Owner acceptance — it is NOT closed. The remaining slices (relational queue rows, room-scoped reorder/vote endpoints, cross-process safety, and migration of the legacy global `queue_state` into a room) are deferred per the 012 document. The original "R08 placeholder" (public read-only API and CORS) was renumbered away from R07b and remains a future sprint per the sequence doc.
