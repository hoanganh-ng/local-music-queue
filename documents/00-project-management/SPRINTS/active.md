...
Sprint 012 / R07d — Room-scoped playback queue (one room-scoped mutation: POST /api/rooms/{slug}/queue/prioritize, plus the matching per-room WebSocket delta and a minimal RoomView Prioritize control) is **in progress on `dev`** — implementation landed on 2026-06-29, Product Owner acceptance is pending. R07d does not advance any out-of-scope work (no global route, no priority balances, no relational rows). See the *Implementation summary (R07d)* section in [012-room-scoped-playback-queue.md](./012-room-scoped-playback-queue.md).
