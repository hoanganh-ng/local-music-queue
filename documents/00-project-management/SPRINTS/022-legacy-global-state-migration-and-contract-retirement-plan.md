...
- R14b ships the **runtime server startup flag `--room-cutover-authoritative`** (read once at process composition; default `false`; in R14b this is `false` in every deployment). The flag is a Go runtime arg parsed at `main()` startup; it is **NOT a build-time-only constant**. At composition, R14b reads the flag and configures two independent behaviors:
  - The legacy-route registration set (registered vs unregistered; tombstone vs real handlers).
  - The `R09iActivityRepository` dependency: the `RoomActivityRepository` interface implementation injected into the room runtime is either a **no-op writer** (when the flag is `false`) or the **real `room_activities`-writing implementation** (when the flag is `true`). When `false`, the legacy global routes continue to serve from the legacy repositories **unchanged** AND any room-runtime `AddActivity` call site that was wired by R09i is a no-op.
...
- **`0010_drop_legacy_global_tables.down.sql` recreates ONLY the four dropped tables with their exact legacy definitions** so that rolling forward/backward on the dropped tables is a no-op on structure. Specifically, the `.down.sql` MUST recreate **only**:
  ...
  - `activities` — recreated with its OWN `BIGSERIAL` sequence (`activities_id_seq`) and the column structure from `0001_initial.up.sql` (with `room_id` cast either to legacy semantics or whatever the current schema carries; mirroring `0001_initial.up.sql`).
